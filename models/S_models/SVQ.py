...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len:Optional[int]=1024, d_k:Optional[int]=None, d_v:Optional[int]=None, norm:str='BatchNorm', attn_dropout:float=0., 
        ...
        # load parameters
        c_in = configs.feature_dim
        self.enc_in = configs.feature_dim
        context_window = configs.input_len
        target_window = configs.pred_len
        n_layers = configs.e_layers
        n_heads = configs.n_heads_i
        d_model = configs.d_model_i
        d_ff = configs.d_ff
    ...
    def forward(self, x, batch_x_mark, dec_inp, batch_y_mark, vq_details=True):
        ...
        x = x.permute(0,2,1)
        x, loss, enc_out, vq_details_lst = self.model(x, vq_details=vq_details)     # enc_out : [bsz  nvars  d_model  patch_num]
    
        x = x.permute(0,2,1)                                                        # x: [bsz  pred len  nvars]
        enc_out = enc_out.permute(0, 1, 3, 2)                                       # enc_out : [bsz  nvars  patch_num  d_model]
