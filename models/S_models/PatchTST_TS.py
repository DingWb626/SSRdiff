...
from models.dm_layers.decouple import series_decomposition, FourierLayer
...
class encoder(nn.Module):
    def __init__(self, configs):
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(
                        ...
                        configs.d_model_i, configs.n_heads_i),
                    configs.d_model_i,
                    configs.d_ff,
            ...
            norm_layer=nn.Sequential(Transpose(1,2), nn.BatchNorm1d(configs.d_model_i), Transpose(1,2))
        )
...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.patch_embedding = PatchEmbedding(
            configs.d_model_i, patch_len, stride, padding, configs.padding_patch,configs.dropout)

        # Series decomposition
        self.decomp = series_decomp(kernel_size=configs.kernel_size) if self.decomposition else None
        self.seasonal = FourierLayer(d_model=configs.d_model_i, factor=configs.fourier_factor) if self.decomposition else None

        # Encoder
        self.encoder = encoder(configs)

        # Prediction Head
        self.head_nf = configs.d_model_i * \
                       int((configs.input_len - patch_len) / stride + 2)

        self.head = FlattenHead(configs.feature_dim, self.head_nf, configs.pred_len,
